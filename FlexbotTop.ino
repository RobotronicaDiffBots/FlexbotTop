...
void setColour(rgb24_t rgbcolour, float brightness) {
  ...
  digitalWrite(IRled_pin, IRLedState);
}
...
void process() {
  ...
  if (radioMessage.type <= 5) {
    ...
    if (radioMessage.type == 123) {
      // The values coming in are from 0 to 180, with 90 as the mid point and the "zero" of the control sticks
      servoDemands[0] = radioMessage.d1;
      servoDemands[1] = radioMessage.d2;
      servoDemands[2] = radioMessage.d3;
      setServos();
    }
  }
